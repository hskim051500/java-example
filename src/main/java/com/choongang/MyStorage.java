...
//     */
    ...
    static void prod_input(Scanner s){
        ...
        if(check ==0){
            System.out.println("[System] 더 이상 등록할 공간이 없습니다." );
        }
        if(check>0) {
            System.out.print("[System] 제품 등록을 원하는 제품명을 입력하세요 :");
            products[flag] = s.nextLine();
            counts[flag]++;
            flag++;
    ...
    public static void prod_amount_add(Scanner s) {
        ...
        String cnt = s.nextLine();
            if(cnt.contains("0")||cnt.contains("1")||cnt.contains("2")||cnt.contains("3")||cnt.contains("4")||cnt.contains("5")
            ||cnt.contains("6")||cnt.contains("7")||cnt.contains("8")||cnt.contains("9")){
    ...
    public static void prod_amount_decrease(Scanner s) {
        ...
        System.out.println("[System] 제품의 출고를 진행합니다.(출고)");
        System.out.println("[System] 현재 등록된 제품 및 수량 ▼");
        for(int i = 0 ; i< products.length;i++){
            System.out.println(">"+products[i]+":"+counts[i]+"개");
        }
        ...
        if(counts[foundIdx] < Integer.parseInt(cnt)){
            System.out.println("[Error] 출고량이 너무 많습니다.");
            return ;
        }
        counts[foundIdx] -= Integer.parseInt(cnt);  // 제품의 수량을 차감
